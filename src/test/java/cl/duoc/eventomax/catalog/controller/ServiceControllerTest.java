...
class ServiceControllerTest {
    ...
    @Test
    void shouldRejectRateWithTooManyIntegerDigits() throws Exception {
        String invalidJson = "{ \"name\": \"Test\", \"rate\": 10000000000.00, \"active\": true }";
        mockMvc.perform(post("/api/catalog/services")
                .contentType(MediaType.APPLICATION_JSON)
                .content(invalidJson))
                .andExpect(status().isBadRequest());
    ...
    @Test
    void shouldRejectRateWithTooManyFractionDigits() throws Exception {
        String invalidJson = "{ \"name\": \"Test\", \"rate\": 1.234, \"active\": true }";
        mockMvc.perform(post("/api/catalog/services")
                .contentType(MediaType.APPLICATION_JSON)
                .content(invalidJson))
                .andExpect(status().isBadRequest());
    ...
    @Test
    void shouldRejectRateWithInvalidScale() throws Exception {
        String invalidJson = "{ \"name\": \"Test\", \"rate\": 0.001, \"active\": true }";
        mockMvc.perform(post("/api/catalog/services")
                .contentType(MediaType.APPLICATION_JSON)
                .content(invalidJson))
                .andExpect(status().isBadRequest());
    ...
    @Test
    void shouldAcceptValidRateMinimum() throws Exception {
        ServiceResponse response = new ServiceResponse(1L, "Test", "Desc", new BigDecimal("0.01"), true);
        when(catalogService.createService(any(ServiceRequest.class))).thenReturn(response);

        String jsonRequest = "{ \"name\": \"Test\", \"rate\": 0.01, \"active\": true }";
        mockMvc.perform(post("/api/catalog/services")
                .contentType(MediaType.APPLICATION_JSON)
                .content(jsonRequest))
                .andExpect(status().isCreated());
    ...
    @Test
    void shouldAcceptValidRateMaximum() throws Exception {
        ServiceResponse response = new ServiceResponse(1L, "Test", "Desc", new BigDecimal("9999999999.99"), true);
        when(catalogService.createService(any(ServiceRequest.class))).thenReturn(response);

        String jsonRequest = "{ \"name\": \"Test\", \"rate\": 9999999999.99, \"active\": true }";
        mockMvc.perform(post("/api/catalog/services")
                .contentType(MediaType.APPLICATION_JSON)
                .content(jsonRequest))
                .andExpect(status().isCreated());
    ...
    void shouldReturn400OnMalformedJson() throws Exception {
        String malformedJson = "{ \"name\": \"Test\", \"rate\": 100.0, ";
        mockMvc.perform(post("/api/catalog/services")
                .contentType(MediaType.APPLICATION_JSON)
                .content(malformedJson))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("Request body is missing or invalid."));
        org.mockito.Mockito.verify(catalogService, org.mockito.Mockito.never()).createService(any());
    }

    @Test
    void shouldReturn400OnMethodArgumentTypeMismatch() throws Exception {
        mockMvc.perform(get("/api/catalog/services/abc"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("Invalid parameter type: id"));
        org.mockito.Mockito.verify(catalogService, org.mockito.Mockito.never()).getServiceById(any());
    }

    @Test
    void shouldReturn415OnUnsupportedMediaType() throws Exception {
        String jsonRequest = "{ \"name\": \"Test\", \"rate\": 100.0, \"active\": true }";
        mockMvc.perform(post("/api/catalog/services")
                .contentType(MediaType.TEXT_PLAIN)
                .content(jsonRequest))
                .andExpect(status().isUnsupportedMediaType())
                .andExpect(jsonPath("$.error").value("Unsupported media type. Please use application/json."));
        org.mockito.Mockito.verify(catalogService, org.mockito.Mockito.never()).createService(any());
    }

    @Test
    void shouldReturn500AndHideExceptionDetailsOnUnexpectedError() throws Exception {
        when(catalogService.listServices()).thenThrow(new RuntimeException("Super secret database error"));

        mockMvc.perform(get("/api/catalog/services"))
                .andExpect(status().isInternalServerError())
                .andExpect(jsonPath("$.error").value("Ocurrió un error inesperado. Por favor, contacte al administrador."))
                .andExpect(jsonPath("$.error", org.hamcrest.Matchers.not(org.hamcrest.Matchers.containsString("Super secret"))));
    }
}
